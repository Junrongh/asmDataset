...
micrographs = db.query(Micrograph).filter(
    Micrograph.primary_microconstituent.in_(c)).all()
...
for micrograph in micrographs:
    ...
    path = 'data/micrographs/micrograph{}{}'.format(
        micrograph.micrograph_id, ext)
    print('{}: {}'.format(micrograph.primary_microconstituent, path))
    full[micrograph.micrograph_id] = path

    for crop in ('UL', 'UR', 'LL', 'LR'):
        path = 'data/crops/micrograph{}-crop{}.tif'.format(
            micrograph.micrograph_id, crop)
